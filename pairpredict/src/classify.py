...
## TODO: clean away things we don't need
...
def get_data(label_file, feature_file, le=None, dv=None) : 
...
def train_classifier(X, Y,C=1,le=None, penalty='elasticnet', l1_ratio=0.15):
    clf = SGDClassifier(loss='log', penalty='elasticnet', alpha=C, l1_ratio=l1_ratio, 
                        class_weight='balanced', random_state=0, fit_intercept=False)
    clf.fit(X,Y)
    return clf
...
if __name__ == '__main__' : 
    ...
    if opts.predict or opts.print_features : 
        ...
        le = bundle['le']
        dv = bundle['dv']
        y, X, _, _, nm = get_data(label_file, feature_file, le, dv)
    else : y, X, le, dv, nm = get_data(label_file, feature_file)
    sy = None; sX = None;
    if opts.tune: 
